...
public class QueryParser {
    ...
    public CatalogParsedQuery parse(String query) {
        ...
        // 기본 단순 검색
        return CatalogParsedQuery.simple(trimmedQuery);
    }
    
    private CatalogParsedQuery parseOrQuery(String queryString) {
        ...
        return CatalogParsedQuery.or(keyword1, keyword2, queryString);
    }
    
    private CatalogParsedQuery parseNotQuery(String queryString) {
        String[] parts = queryString.split("-", 2);
        if (parts.length != 2) {
        ...
        return CatalogParsedQuery.not(includeKeyword, excludeKeyword, queryString);
    }
}